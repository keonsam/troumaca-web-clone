...
class AssetTypeDbRepository {
    saveAsset(asset) {
        ...
        return rxjs_1.default.Observable.create(function (observer) {
            db_js_1.assets.insert(asset, function (err, doc) {
                if (err) {
                    observer.error(err);
                }
                else {
    ...
    getAssets(pageNumber, pageSize, order) {
        return rxjs_1.default.Observable.create(function (observer) {
            ...
            db_js_1.assets.find({}).sort(order).skip(skip).limit(pageSize).exec(function (err, doc) {
                if (!err) {
                    observer.next(doc);
                }
                else {
    ...
    getAssetCount() {
        return rxjs_1.default.Observable.create(function (observer) {
            db_js_1.assets.count({}, function (err, count) {
                if (!err) {
                    observer.next(count);
                }
                else {
    ...
    getAssetById(assetId) {
        return rxjs_1.default.Observable.create(function (observer) {
            ...
            db_js_1.assets.findOne(query, function (err, doc) {
                if (!err) {
                    observer.next(doc);
                }
                else {
    ...
    updateAsset(assetId, asset) {
        return rxjs_1.default.Observable.create(function (observer) {
            let query = {};
            query["assetId"] = assetId;
            db_js_1.assets.update(query, asset, {}, function (err, numReplaced) {
                if (!err) {
    ...
    deleteAsset(assetId) {
        return rxjs_1.default.Observable.create(function (observer) {
            let query = {};
            query["assetId"] = assetId;
            db_js_1.assets.remove(query, {}, function (err, numRemoved) {
                if (!err) {
