...
export class OrganizationRepositoryAdapter extends OrganizationRepository {
  ...
  public sendOrganizationRequest(request: string): Observable<boolean> {
    return this.organizationClient.sendOrganizationRequest(request);
  }
  ...
  public addOrganization(organization: Organization, type?: string): Observable<Organization> {
    return this.organizationClient
      .addOrganizationState(mapObjectProps(organization, new OrganizationState()), type)
      .pipe(map(value => {
        return mapObjectProps(value, new Organization());
      }));
  }
